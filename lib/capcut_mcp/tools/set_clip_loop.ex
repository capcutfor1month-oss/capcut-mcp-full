defmodule CapcutMcp.Tools.SetClipLoop do
  @moduledoc "MCP tool: enable or disable looping on a clip/segment."
  @behaviour CapcutMcp.Tool

  alias CapcutMcp.Tools.{SegmentMutation, ToolArgs}

  @impl true
  def definition do
    %{
      "name" => "set_clip_loop",
      "description" =>
        "Enables or disables looping on a clip/segment. When enabled, the clip repeats for its full timeline duration.",
      "inputSchema" => %{
        "type" => "object",
        "properties" => %{
          "project_id" => %{"type" => "string", "description" => "The draft_id of the project"},
          "clip_id" => %{
            "type" => "string",
            "description" => "The segment ID (from get_timeline)"
          },
          "loop" => %{
            "type" => "boolean",
            "description" => "true to enable loop, false to disable"
          }
        },
        "required" => ["project_id", "clip_id", "loop"]
      }
    }
  end

  @impl true
  def execute(%{"project_id" => id, "clip_id" => clip_id, "loop" => loop})
      when is_boolean(loop) do
    SegmentMutation.run(id, clip_id, &Map.put(&1, "is_loop", loop),
      success: "Loop #{if loop, do: "enabled", else: "disabled"} on segment #{clip_id}."
    )
  end

  def execute(%{"loop" => loop}),
    do: {:error, "Invalid loop value: #{inspect(loop)} (must be boolean)"}

  def execute(args),
    do: {:error, ToolArgs.missing_required_message(args, ["project_id", "clip_id", "loop"])}
end
